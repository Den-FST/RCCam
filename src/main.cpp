...
const int fwdPin2 = 14;  //Reverse Motor Pin
const int fwdPinB = 15;  //Forward Motor Pin
const int fwdPinB2 = 13;  //Reverse Motor Pin*/
...
void setup()
{
  ...
  ledcSetup(2, 100, 12); //channel, freq, resolution
  ledcAttachPin(fwdPin, 2); // pin, channel
   // reverse motor PWM 
   ledcSetup(14, 100, 12); //channel, freq, resolution
  ledcAttachPin(fwdPin2, 14); // pin, channel
#ifdef MotorB 
    // forward motor PWM
  ledcSetup(13, 200, 12); //channel, freq, resolution
  ledcAttachPin(fwdPin, 14); // pin, channel
   // reverse motor PWM
   ledcSetup(15, 200, 12); //channel, freq, resolution
  ledcAttachPin(fwdPin2, 15); // pin, channel
#endif  */
  // steering servo PWM
  ledcSetup(12, 50, 16); //channel, freq, resolution
  ledcAttachPin(turnPin, 12); // pin, channel
 
/*
  fwdAnalogWrite(2, 0); // stop
  fwdAnalogWrite(14, 0); // stop
  #ifdef MotorB 
  fwdAnalogWrite(13, 0); // stop
  fwdAnalogWrite(15, 0); // stop
#endif 
*/
...
void handle_message(WebsocketsMessage msg) {
  ...
  int SteerValMap = map(steerValue, -90, 90, 30, 150);

  Serial.print("SteerValMap: ");
  Serial.print(SteerValMap);
  Serial.print(" --  ");

  if (steerValue > 5 ) {
    
    steeringAnalogWrite(12, SteerValMap);
  }
   ...
   steeringAnalogWrite(12, SteerValMap);
    ...
    steeringAnalogWrite(12, 85); // center steering
  }
  int fw2map = forwardValue;
  forwardValue = map(forwardValue, 0, -90, 0, 255);
  int forwardValue2 = map(fw2map, 0, 90, 0, 255);
  Serial.print("FowValMAP: ");
  Serial.print(forwardValue);
  Serial.print(" --  ");
  Serial.print("FowValMAP2: ");
  Serial.println(forwardValue2);
  if (fw2map < -5 ) {
    motorA.motorGo(forwardValue);  
    /*
    fwdAnalogWrite(14, 0); 
    fwdAnalogWrite(2, forwardValue);
    #ifdef MotorB 
    fwdAnalogWrite(13, 0); 
    fwdAnalogWrite(15, forwardValue2);
    #endif*/
  }
  else if (fw2map > 5 ) {
   motorA.motorRev(forwardValue2); 
    /*
    fwdAnalogWrite(2, 0); 
    fwdAnalogWrite(14, forwardValue2);
    #ifdef MotorB 
    fwdAnalogWrite(15, 0); 
    fwdAnalogWrite(13, forwardValue2);
    #endif*/
  }
  else {
    motorA.motorStop();  
